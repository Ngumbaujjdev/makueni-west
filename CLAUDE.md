...
- **Read-only detail views (modals, summary panels) use icon + pill accents, not uniform flat text.** Every field in a detail view shouldn't look identical — pair each label with a small solid-color icon avatar (`avatar avatar-sm/md avatar-rounded bg-{color} text-white`, the same pattern `renderStatCard` uses) and render categorical or count-like values as solid color pills (`badge bg-{color}`, the same pattern `renderStatusBadge` uses), not plain black text. Reserve plain text for genuinely free-form values (notes, timestamps) and one large plain "hero" number per view for the headline stat. Use `DemographicsUI.renderDetailField()` (`assets/js/pages/demographics/ui-helpers.js`) rather than hand-rolling label/value markup.
